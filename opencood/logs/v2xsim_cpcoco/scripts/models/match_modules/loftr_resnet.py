...
from opencood.models.match_modules.consistency import NonLocalNet
from opencood.utils.matcher_utils import get_fore_point, get_voxel_coordinate, noise_transformation
...
class OverlapHead(nn.Module):
    def __init__(self, middle_dim):
        ...
        self.sigma_spat = nn.Parameter(torch.Tensor([0.1]).float(), requires_grad=False)
    ...
    def forward(self, feat_c, mask, grid):
        ...
        with torch.no_grad():
            topk_value, topk_index = torch.topk(match_logits.clone(), 5, dim=1)
            ...
            match_valid = torch.sum(corr_compatibility > 0.8, dim=1)
