...
def get_metrics(metric_dict):
    ...
    for pts0, pts1, pts1_transed, corr_index, valid_corr, t_matrix, pair in zip(metric_dict['point0'], metric_dict['point1'], metric_dict['point1_transed'], 
        ...
        if len(src_p) == len(tgt_p) <= 10:
            ...
            if success: 
                correct_sum += 1
                # print(lidar_np[f'{pair[0]+1}']['lidar_name'], rte, rre)
                # dist = torch.norm(src_p - tgt_transed, dim=1) < 1.0
                # print('inlier rate:', (sum(dist.int())/len(dist)).item())
                # from opencood.utils.visual_utils import plot_corresponding
                # plot_corresponding(lidar_np[f'{pair[0]+1}']['lidar_np'][:, :2], 
                #                    lidar_np[f'{pair[1]+1}']['lidar_np'][:, :2],
                #                     src_p,
                #                     tgt_p,
                #                     [(i, i, dist[i]) for i in range(len(src_p))],
                #                     prefix= lidar_np[f'{pair[0]+1}']['lidar_name'],
                #                     pair = pair)
