...
from opencood.models.match_modules.cpscoco_matcher import CPSCoCoMatcher
from opencood.utils.transformation_utils import normalize_pairwise_tfm
from opencood.utils.metric_utils import get_metrics
...
class PointPillarBaselineMultiscale(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cpscoco_matcher = CPSCoCoMatcher(args['cpscoco_matcher'])
    ...
    def forward(self, data_dict):
        ...
        # cpscoco matcher
        if not record_len[0] == 1:
            pair = torch.tensor(list(combinations(range(spatial_features.shape[0]), 2)), device=spatial_features.device).t()
            inference_data, loss_cpscoco = self.cpscoco_matcher(spatial_features, 
                                                batch_dict['voxel_coords'].clone().detach().long(), 
                                                normalized_affine_matrix,
                                                pair,
                                                is_train = data_dict.get('is_train', True)
                                                )

            if inference_data:
                inference_data.update({'trans_matrix': data_dict['pairwise_t_matrix'][0, pair[0], pair[1]],
                                       'pair': pair, 'voxel': self.voxel_size[0]})
                # match_time = [inference_data.pop('time') / pair.shape[1]]

                # inference_data.update({'lidar_np': data_dict['lidar_dict']})
                valid_matrix, valid_pair, metrics_dict = get_metrics(inference_data)
                valid_pair = valid_pair.astype(int)
                valid_index = [0] 
                valid_index += valid_pair[1, valid_pair[0, :] == 0].tolist()
                
                spatial_features = spatial_features[valid_index]
                normalized_affine_matrix = valid_matrix[:, valid_index][:, :, valid_index]
                record_len = torch.zeros_like(record_len) + len(valid_index)

                post_time = metrics_dict['time_post']

                output_dict.update({'inference_dict': metrics_dict})

            output_dict.update({'cpscoco_dict': loss_cpscoco})
        else: 
            match_time = []
            post_time = []

        start_fus_dec = time.time() * 1000
        # multiscale fusion
        ...
        end_fus_dec = time.time() * 1000

        output_dict.update({'cls_preds': psm,
                            'reg_preds': rm,
                            #'times': [match_time, end_fus_dec-start_fus_dec, post_time]
                            })
