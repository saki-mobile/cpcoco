...
class PointPillarBaselineMultiscale(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if not record_len[0] == 1:
            ...
            kpts_candidate, loss_cpscoco = self.cpscoco_matcher(spatial_features, 
                                                                data_dict['inputs_m1']['voxel_coords'].clone().detach().long(), 
                                                                normalized_affine_matrix,
                                                                pair, 
                                                                is_train = data_dict.get('is_train', True)
                                                                )

            output_dict.update({'loss_dict': loss_cpscoco})
