...
def add_noise_data_dict(data_dict, noise_setting):
    ...
    if noise_setting['add_noise']:
        for cav_id, cav_content in data_dict.items():
            ...
            raw_pose = cav_content['params']['lidar_pose']
            if noise_setting['args']['rot_std'] == 0:
                noise_setting['args']['rot_std'] = np.pi

            if "laplace" in noise_setting['args'].keys() and noise_setting['args']['laplace'] is True:
                noise_level = generate_noise_laplace( # we just use the same key name
                                                            noise_setting['args']['pos_std'],
                                                            noise_setting['args']['rot_std'],
                                                            noise_setting['args']['pos_mean'],
                                                            noise_setting['args']['rot_mean']
                                                        )
                cav_content['params']['lidar_pose'] = cav_content['params']['lidar_pose'] + noise_level                                  
            else:
                noise_level = generate_noise(
                                            noise_setting['args']['pos_std'],
                                            noise_setting['args']['rot_std'],
                                            noise_setting['args']['pos_mean'],
                                            noise_setting['args']['rot_mean']
            ...
            noise_lidar_np = x1_to_x2(raw_pose, cav_content['params']['lidar_pose']) @ np.concatenate((cav_content['lidar_np'][:, :3], np.ones_like(cav_content['lidar_np'][:, 0:1])), axis=-1).T
            noise_lidar_np = np.concatenate((noise_lidar_np.T[:, :3], cav_content['lidar_np'][:, -1:]), axis=1, dtype=np.float32)
            cav_content['lidar_np'] = noise_lidar_np
