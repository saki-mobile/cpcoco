...
from opencood.loss.match_loss import MatchLoss
...
class PointPillarLoss(nn.Module):
    def __init__(self, args):
        ...
        self.match_loss = MatchLoss(args['match'])
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        self.loss_dict.update({'reg_loss': reg_loss.item(),
        ...
        ######## Match ###########
        total_match_loss = torch.tensor(0.).cuda()
        point_circle_loss = torch.tensor(0.).cuda()
        point_overlap_loss = torch.tensor(0.).cuda()
        match_dict = output_dict.get('cpscoco_dict', None)
        if match_dict is not None:
            match_loss_dict, point_circle_loss, point_overlap_loss = self.match_loss(match_dict)
            total_match_loss = point_circle_loss + point_overlap_loss
            total_loss += total_match_loss
            self.loss_dict.update(match_loss_dict)

        self.loss_dict.update({'total_loss': total_loss.item()})
        return total_loss, reg_loss + cls_loss + dir_loss, total_match_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix=""):
        ...
        circle_loss = self.loss_dict.get('circle_loss', 0)
        overlap_loss = self.loss_dict.get('overlap_loss', 0)


        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || Circle Loss: %.4f || Overlap Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, circle_loss, overlap_loss))
        ...
        if not writer is None:
            ...
            writer.add_scalar('Dir_loss'+suffix, dir_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Circle_loss'+suffix, circle_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Overlap_loss'+suffix, overlap_loss,
                            epoch*batch_len + batch_id)
